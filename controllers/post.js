...
module.exports.updatePost = async (req, res, next) => {
    try {
        ...
        const currentPost = await db.Post.findOne({ where: { id, userId: req.user.userId } });
        if (currentPost) {
            return await db.sequelize.transaction(async (t) => {
                const promises = [];
                const updatedPost = currentdb.Post.update(post, { transaction: t });
                promises.push(updatedPost);

                const deletedTags = db.PostsTags.destroy({ where: { PostId: id } });
...
module.exports.deletePost = async (req, res, next) => {
    try {
        const id = +req.params.id;
        const deleted = await db.Post.destroy({ where: { id, UserId: req.user.userId } });
        if (deleted > 0) {
...
async function createPost(post, tagsString) {
    try {
        return await db.sequelize.transaction(async (t) => {
            const postResult = await db.Post.create(post, { transaction: t });
            ...
            for (let i = 0; i < tagCombinations.length; i++) {
                ...
                tagFetchingPromises.push(db.Tag.findOne({ where: { name }, attributes: ['id'] }));
            }
            ...
            if (tagInsertions.length > 0) {
                const tagInsertionResults = await db.Tag.bulkCreate(
                    tagInsertions,
                    {
                        transaction: t,
                        returning: true,
            ...
            await db.PostsTags.bulkCreate(postTags, { transaction: t, raw: true });
